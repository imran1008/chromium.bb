...
IntSize CachedImage::imageSizeForRenderer(const RenderObject* renderer, float multiplier)
{
    ...
    if (m_image->isSVGImage()) {
        ...
        if (sizeAndScales.size.isEmpty())
            return m_image->size();
        if (sizeAndScales.zoom == 1)
            return sizeAndScales.size;
        if (multiplier == 1) {
            // Consumer wants unscaled coordinates.
            sizeAndScales.size.setWidth(sizeAndScales.size.width() / sizeAndScales.zoom);
            sizeAndScales.size.setHeight(sizeAndScales.size.height() / sizeAndScales.zoom);
            return sizeAndScales.size;
        }
        return sizeAndScales.size;
    }
